...
public class GroupServiceImpl implements GroupService {
    ...
    public GroupDto createGroup(GroupCreateRequest req) {
        ...
        if (image != null && !image.isEmpty()) {
            try {
                ...
                // 🚨 여기를 수정: URL에 /uploads/group/ 서브 디렉토리를 포함시킵니다.
                imageUrl = "/uploads/group/" + filename; // <-- /uploads/group/ 으로 변경
            } catch (IOException e) {
    ...
    public GroupDto updateGroup(Integer groupId, GroupUpdateRequest req) {
        ...
        // 💡 주의: 현재 updateGroup 메서드에는 이미지 변경 로직이 없습니다.
        // 만약 이미지 변경 기능을 추가하려면, GroupUpdateRequest에 MultipartFile 필드를 추가하고
        // UserServiceImpl의 updateUser 메서드처럼 이미지 업데이트 로직을 구현해야 합니다.
