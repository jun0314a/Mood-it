// src/main/java/com/example/group/controller/GroupController.java
package com.example.group.controller;

// import com.example.auth.jwt.JwtUtil; // JwtUtil 사용 안 한다면 제거
...
import org.springframework.web.multipart.MultipartFile; // 이 줄은 사용 안 하지만, 혹시 모를 MultipartFile 파라미터에 대비해서 유지
...
public class GroupController {
    ...
    // private final ObjectMapper objectMapper; // @ModelAttribute를 사용하면 필요 없습니다.
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "201", description = "그룹 생성 성공",
                    content = @Content(mediaType = "application/json",
                            ...
                            examples = @ExampleObject(value = "{\"id\":1,\"groupName\":\"스터디 그룹\",\"description\":\"함께 공부하는 모임\",\"groupImageUrl\":\"/group/uuid.png\"}"))),
            @ApiResponse(responseCode = "400", description = "유효하지 않은 입력"),
            @ApiResponse(responseCode = "401", description = "인증 실패")
    })
    @PostMapping(consumes = MediaType.MULTIPART_FORM_DATA_VALUE) // consume 타입을 multipart/form-data로 명시
    public ResponseEntity<GroupDto> createGroup(
            // creatorId는 GroupCreateRequest 내부에 있으므로 @RequestParam으로 별도 받을 필요 없음 (클라이언트가 FormData에 같이 보낼 것)
            @ModelAttribute @Valid GroupCreateRequest request) { // <-- @RequestBody 대신 @ModelAttribute 사용
